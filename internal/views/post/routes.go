...
func SetupPostRoutes(){
	...
	http.HandleFunc("POST /api/post", middleware.ValidateSessionMiddleware(CreatePostHandler))
	http.HandleFunc("POST /api/post/{id}/comments", middleware.ValidateSessionMiddleware(CreateCommentHandler))
	http.HandleFunc("POST /api/like/{id}", middleware.ValidateSessionMiddleware(CreateLikeHandler))
